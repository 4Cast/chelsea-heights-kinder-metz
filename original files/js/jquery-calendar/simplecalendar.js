var calendar = 
{
	init: function(ajax) 
	{
		if (ajax) 
		{
			...
			$.ajax
			({
  				//url			: 	'js/jquery-calendar/data/events.json',
  				url			: 	'js/jquery-calendar/data/get-events.php',
  				dataType	:	"json",
  				type		: 	'GET',
			})
  			.done(function(data) 
  			{
  				...
  				{
  					$('.list').append('<div class="hidden day-event" date-day="'+ events[i].day +'" date-month="' + events[i].month +'" date-year="'+ events[i].year +'" data-number="'+ i +'"><p class="event-date">'+events[i].display_date+'</p><p class="event-title"><a href="events/'+ events[i].url +'">'+ events[i].title +'</a></p></div>');
  				}
